...
if save_figs:
    fig.savefig(
        os.path.join(fig_path, "pwl_sweep_centered_at_max.pdf"), bbox_inches="tight"
    )
...
def plot_polarization_data(
    wavelengths, pol_data, sig_wl_idxs, idler_wl_idxs, title_args
):
    fig, ax = plt.subplots()
    ...
    axins1, axins2 = [
        ax.inset_axes([0.3, 0.6, 0.3, 0.3]),
        ax.inset_axes([0.7, 0.6, 0.3, 0.3]),
    ]
    ...
    for axins, idx_range in zip(insets, idx_ranges):
        for data in pol_data:
            ...
            axins.plot(
                ...
                mean_pol[idx_range[0] : idx_range[1]],
            )
            axins.fill_between(
                wavelengths[idx_range[0] : idx_range[1]],
                mean_pol[idx_range[0] : idx_range[1]]
                - std_pol[idx_range[0] : idx_range[1]],
                mean_pol[idx_range[0] : idx_range[1]]
                + std_pol[idx_range[0] : idx_range[1]],
                alpha=0.3,
            )
...
for pump_sep_num in range(0, 5):
    ...
    plot_polarization_data(
        wavelengths, max_min_data, sig_wl_idxs, idler_wl_idxs, title_args
    )
    if save_figs:
        plt.savefig(
            f"{fig_path}/{fig_path_extention}/{np.unique(pump_separations)[pump_sep_num]}nm.pdf",
            bbox_inches="tight",
        )
