...
tmp_pump = pump_data["100KHz"]
df = pd.DataFrame(
    {
        "Time ax": np.array(tmp_pump["time_ax"])[0, :],
        "Voltage 0.1": np.array(tmp_pump["voltage"])[0, :],
        "Voltage 0.2": np.array(tmp_pump["voltage"])[1, :],
        "Voltage 0.25": np.array(tmp_pump["voltage"])[2, :],
        "Voltage 0.5": np.array(tmp_pump["voltage"])[3, :],
        "Voltage 0.75": np.array(tmp_pump["voltage"])[4, :],
    }
)
ref_df = pd.DataFrame(
    {
        "Time ref": np.array(tmp_pump["time_ax_ref"])[0, :],
        "Voltage ref": np.array(tmp_pump["voltage_ref"])[0, :],
    }
)
# df.to_csv("100KHz_multiple_duty_cycles.csv")
# ref_df.to_csv("100KHz_ref.csv")
...
ax.plot(pump_time * 10**6, pump_voltage_normalized**2, label="Pump squared")
ax.set_xlabel(r"Time ($\mu$s)")
ax.set_ylabel("Voltage (a.u.)")
ax.legend()
idler_df = pd.DataFrame(
    {
        "Time": idler_time_shifted,
