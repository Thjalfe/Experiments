...
ando_pow_start = 0
pump1_laser = AndoLaser(pump1_start, GPIB_address=24, power=8, GPIB_bus=1)
...
slope = 0.2
start_tisa_wl = 945.9
toptica_wl_at_start_tisa = 965.069
...
tisa_wl_array = np.arange(945.9, 960.9, 1)
...
data_file = r"tisa-wl=944.9-873.9nm_toptica_fine_sweep_1700+nm-idler.pkl"
...
fit = linregress(
    old_data_for_fit["tisa_wl_array"][:5],
    old_data_for_fit["optimum_vals_dict"]["toptica_wls"][:5],
)
...
for tisa_idx, tisa_wl in enumerate(tisa_wl_array):
    ...
    plot_spectra(spectra, title=f"TISA WL: {tisa_wl} nm")
    plt.show()
...
idx = 4
interesting_tisa_wl = measured_data["tisa_wl_array"][idx]
interesting_tisa_wl = 914.9
tisa.set_wavelength(interesting_tisa_wl)
telecom_wl = 1580
telecom_wl_diff_factor = (1590 - telecom_wl) / 10
interesting_toptica_wls = 966.025 - 0.2
pump1_laser.wavelength = telecom_wl
# 914.9, 965.825
# 915.9, 966.025
# 916.9, 966.255
